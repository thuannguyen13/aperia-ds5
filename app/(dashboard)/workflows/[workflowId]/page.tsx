...
const taskStatusStyle: Record<TaskStatus, { badgeClass: string; icon: React.ElementType; iconColor: string; bg: string }> = {
  COMPLETED:   { badgeClass: "border-transparent bg-green-100 text-green-800 hover:bg-green-100",  icon: CheckCircle2, iconColor: "text-green-500",  bg: "bg-green-50"  },
  IN_PROGRESS: { badgeClass: "border-transparent bg-blue-100 text-blue-800 hover:bg-blue-100",    icon: Clock,        iconColor: "text-blue-500",   bg: "bg-blue-50"   },
  PENDING:     { badgeClass: "border-transparent bg-gray-100 text-gray-600 hover:bg-gray-100",    icon: Circle,       iconColor: "text-slate-300",  bg: "bg-white"     },
  OVERDUE:     { badgeClass: "border-transparent bg-red-100 text-red-800 hover:bg-red-100",      icon: Clock,        iconColor: "text-red-500",    bg: "bg-red-50"    },
}

const roleClass: Record<string, string> = {
  OFFICER:       "border-transparent bg-slate-100 text-slate-700 hover:bg-slate-100",
    ...
    <div className="flex flex-col gap-6">
      ...
      <Card>
        <CardContent className="p-5">
          <div className="flex items-start justify-between">
            <div>
              <div className="flex items-center gap-2">
                <Badge variant="outline" className="border-transparent bg-blue-100 text-blue-800 hover:bg-blue-100">{workflow.status.replace("_"," ")}</Badge>
          ...
          <Progress value={progress} className="mt-4 h-2 bg-slate-100" indicatorClassName="bg-blue-500" />
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <Badge variant="outline" className={style.badgeClass}>{task.status.replace("_"," ")}</Badge>
                        </div>
                        <div className="mt-2 flex flex-wrap items-center gap-4 text-xs text-slate-500">
                          ...
                          <Badge variant="outline" className={cn("text-[10px]", roleClass[task.role])}>{task.role.replace("_"," ")}</Badge>
