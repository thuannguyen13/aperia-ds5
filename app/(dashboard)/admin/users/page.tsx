...
export default function AdminUsersPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <TopNav title="Admin — Users" breadcrumb={["Admin", "Users"]} />

      {/* Summary */}
      <div className="grid grid-cols-5 gap-4">
        {(["ADMIN","PROGRAM_LEAD","LEAD_OFFICER","OFFICER","READ_ONLY"] as UserRole[]).map(role => (
          <div key={role} className="flex flex-col gap-1 rounded-lg border border-border bg-white p-3 text-center">
            <span className="text-2xl font-bold text-slate-900">{users.filter(u => u.role === role && u.active).length}</span>
            <span className={`inline-block rounded-full px-2 py-0.5 text-xs font-medium ${roleStyle[role]}`}>{role.replace("_"," ")}</span>
          </div>
        ))}
      </div>
            ...
            <tbody className="divide-y divide-border">
              {filtered.map(u => (
                <tr key={u.id} className={`hover:bg-slate-50 transition-colors ${!u.active ? "opacity-50" : ""}`}>
                  ...
                  <td className="py-2.5 pr-4 text-xs text-slate-500">{u.email}</td>
                  <td className="py-2.5 pr-4">
                    <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${roleStyle[u.role]}`}>{u.role.replace("_"," ")}</span>
                  </td>
                  <td className="py-2.5 pr-4 text-xs text-slate-600">{u.region}</td>
                  <td className="py-2.5 pr-4 text-xs text-slate-500">{u.lob}</td>
                  <td className="py-2.5 pr-4 text-xs text-slate-500 whitespace-nowrap">{u.createdAt}</td>
                  <td className="py-2.5 pr-4 text-xs text-slate-500 whitespace-nowrap">{u.lastLogin}</td>
                  <td className="py-2.5 pr-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowAdd(false)}>
          <div className="w-[480px] rounded-xl bg-white p-6 shadow-xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex flex-col gap-3">
              {[["Full Name","text","John Smith"],["Email","email","john.smith@fiserv.com"]].map(([label, type, placeholder]) => (
                <div key={label}>
                  <label className="text-xs font-medium text-slate-700">{label}</label>
                  <input type={type} placeholder={placeholder} className="mt-1 w-full rounded-lg border border-border px-3 py-2 text-sm placeholder:text-slate-400" />
                </div>
              ))}
              {[
                { label: "Role", opts: ["ADMIN","PROGRAM_LEAD","LEAD_OFFICER","OFFICER","READ_ONLY"] },
